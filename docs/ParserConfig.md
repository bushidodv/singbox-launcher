...
- **Версия 3** (текущая): плоская структура, поля `filters`, `addOutbounds` и `preferredDefault` на верхнем уровне объекта outbound

**Автоматическая миграция**: Конфигурации версий 1 и 2 автоматически мигрируют в версию 3 при загрузке. Подробнее о логике миграции см. раздел [Логика работы мигратора](#логика-работы-мигратора).
      ...
      "outbounds": [
        {
          ...
          // Главный фильтр для выбора узлов (версия 3, необязательно)
...
| `filters`         | object   | Нет          | Главный фильтр для выбора узлов (версия 3). OR между объектами в массиве, AND между ключами внутри объекта. В версии 2 называлось `outbounds.proxies`. |
...
migrateV2ToV3() - если версия 2 (или после v1→v2)
    ...
    ↓
Финальный парсинг в clean ParserConfig (версия 3)
```
...
- **Производительность**: Миграции выполняются только при необходимости, версия 3 парсится напрямую без миграции
